...
class StarterBlog {
	...
	function init_hooks() {
		...
		add_action( 'wp_head', array( $this, 'starterblog_style' ), 2 );
	}
	...
	/**
	 * Main StarterBlog Instance.
	 *
	 * Ensures only one instance of StarterBlog is loaded or can be loaded.
	 *
	 * @return StarterBlog Main instance.
	 */
	...
	function content_width() {
		$GLOBALS['content_width'] = apply_filters( 'starterblog_content_width', 843 );
	}
	...
	function theme_setup() {
		 ...
		 * to change 'starterblog' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'starterblog', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'starter-blog' ),
			)
		);
	...
	function register_sidebars() {
		register_sidebar(
			array(
				'name'          => esc_html__( 'Primary Sidebar', 'starter-blog' ),
				'id'            => 'sidebar-1',
		...
		register_sidebar(
			array(
				'name'          => esc_html__( 'Secondary Sidebar', 'starter-blog' ),
				'id'            => 'sidebar-2',
				'description'   => esc_html__( 'Add widgets here.', 'starter-blog' ),
				'before_widget' => '<section id="%1$s" class="widget %2$s">',
				'after_widget'  => '</section>',
		...
		for ( $i = 1; $i <= 6; $i ++ ) {
			register_sidebar(
				array(
					...
					'name'          => sprintf( __( 'Footer Sidebar %d', 'starter-blog' ), $i ),
					'id'            => 'footer-' . $i,
					'description'   => __( 'Add widgets here.', 'starter-blog' ),
					'before_widget' => '<section id="%1$s" class="widget %2$s">',
					'after_widget'  => '</section>',
					'before_title'  => '<h4 class="widget-title" aria-label="Widget Title">',
					'after_title'   => '</h4>',
	...
	function scripts() {
		...
		if ( ! class_exists( 'StarterBlog_Font_Icons' ) ) {
			require_once get_template_directory() . '/inc/customizer/class-customizer-icons.php';
		}
		StarterBlog_Font_Icons::get_instance()->enqueue();

		$suffix = $this->get_asset_suffix();

		do_action( 'starterblog/load-scripts' );

		$css_files = apply_filters(
			'starterblog/theme/css',
			array(
				'google-font' => StarterBlog_Customizer_Auto_CSS::get_instance()->get_font_url(),
				'style'       => $this->get_style_uri(),
			)
		);

		$js_files = apply_filters(
			'starterblog/theme/js',
			array(
				'starterblog-themejs' => array(
		...
		foreach ( $css_files as $id => $url ) {
			...
			if ( is_array( $url ) ) {
				...
				wp_enqueue_style( 'starterblog-' . $id, $arg['url'], $arg['deps'], $arg['ver'] );
			} elseif ( $url ) {
				wp_enqueue_style( 'starterblog-' . $id, $url, $deps, self::$version );
			}
		}
		...
		wp_add_inline_style( 'starterblog-style', StarterBlog_Customizer_Auto_CSS::get_instance()->auto_css() );
		wp_localize_script(
			'starterblog-themejs',
			'StarterBlog_JS',
			apply_filters( // phpcs:ignore
				...
				array(
					'is_rtl' => is_rtl(),
					'css_media_queries' => StarterBlog_Customizer_Auto_CSS::get_instance()->media_queries,
					'sidebar_menu_no_duplicator' => StarterBlog()->get_setting( 'header_sidebar_menu_no_duplicator' ),
				)
			)
		);

		do_action( 'starterblog/theme/scripts' );
	}

	public function starterblog_style() {
	...
	function init() {
		$this->init_hooks();
		$this->includes();
		$this->customizer = StarterBlog_Customizer::get_instance();
		$this->customizer->init();
		do_action( 'starterblog/init' );
	}

	function get_setting( $id, $device = 'desktop', $key = null ) {
		return StarterBlog_Customizer::get_instance()->get_setting( $id, $device, $key );
	}

	function get_media( $value, $size = null ) {
		return StarterBlog_Customizer::get_instance()->get_media( $value, $size );
	}

	function get_setting_tab( $name, $tab = null ) {
		return StarterBlog_Customizer::get_instance()->get_setting_tab( $name, $tab );
	}
...
// Starter Blog Header Meta
function starterblog_meta_description() {
    global $post;
    if ( is_singular() ) {
...
add_action( 'wp_head', 'starterblog_meta_description');