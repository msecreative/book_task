...
class StarterBlog_Builder_Item_Search_Box {
	...
	function __construct() {
		$this->label = __( 'Search Box', 'starter-blog' );
	}
	...
	function customize() {
		...
		$config   = array(
			...
			array(
				...
				'selector'        => "$selector",
				'render_callback' => $fn,
				'label'           => __( 'Placeholder', 'starter-blog' ),
				'default'         => __( 'Search ...', 'starter-blog' ),
				'priority'        => 10,
			),
			...
			array(
				...
				'label'           => __( 'Search Form Width', 'starter-blog' ),
				'description'     => __( 'Note: The width can not greater than grid width.', 'starter-blog' ),
				'priority'        => 15,
			),
			...
			array(
				...
				'label'           => __( 'Input Height', 'starter-blog' ),
				'priority'        => 20,
			),
			...
			array(
				...
				'label'           => __( 'Icon Size', 'starter-blog' ),
				'priority'        => 25,
			),
			...
			array(
				...
				'label'           => __( 'Icon Position', 'starter-blog' ),
				'priority'        => 30,
			),
			...
			array(
				...
				'label'       => __( 'Input Text Typography', 'starter-blog' ),
				'description' => __( 'Typography for search input', 'starter-blog' ),
				'priority'        => 35,
			),
			...
			array(
				...
				'section'     => $this->section,
				'css_format'  => 'styling',
				'title'       => __( 'Input Styling', 'starter-blog' ),
				'description' => __( 'Search input styling', 'starter-blog' ),
				'selector'    => array(
			...
			array(
				...
				'section'     => $this->section,
				'css_format'  => 'styling',
				'title'       => __( 'Icon Styling', 'starter-blog' ),
				'description' => __( 'Search input styling', 'starter-blog' ),
				'selector'    => array(
	 ...
	 */
	function render() {
		$form_extra_class = apply_filters( 'starterblog/builder_item/search-box/form_extra_class', array() );
		$placeholder = StarterBlog()->get_setting( $this->section . '_placeholder' );
		$placeholder = sanitize_text_field( $placeholder );
		/**
		 * Hook: starterblog/builder_item/search-box/before_html
		 *
		 * @since 1.5.0
		 */
		do_action( 'starterblog/builder_item/search-box/before_html' );
		...
		<form role="search" class="header-search-form <?php echo esc_attr( implode( ' ', $form_extra_class ) ); ?>" action="<?php echo esc_url( home_url( '/' ) ); ?>">
			<div class="search-form-fields">
				<span class="screen-reader-text"><?php echo _x( 'Search for:', 'label', 'starter-blog' ); ?></span>
				<?php
				/**
				 * Hook: starterblog/builder_item/search-box/html_content/before_input
				 *
				 * @since 1.5.0
				 */
				do_action( 'starterblog/builder_item/search-box/html_content/before_input' );
				?>

				<input type="search" class="search-field" placeholder="<?php echo esc_attr( $placeholder ); ?>" value="<?php echo get_search_query(); ?>" name="s" title="<?php echo esc_attr_x( 'Search for:', 'label', 'starter-blog' ); ?>" />

				<?php
				/**
				 * Hook: starterblog/builder_item/search-box/html_content/after_input
				 *
				 * @since 1.5.0
				 */
				do_action( 'starterblog/builder_item/search-box/html_content/after_input' );
				?>
			</div>
		...
		/**
		 * Hook: starterblog/builder_item/search-box/after_html
		 *
		 * @since 1.5.0
		 */
		do_action( 'starterblog/builder_item/search-box/after_html' );
	}
}

StarterBlog_Customize_Layout_Builder()->register_item( 'header', new StarterBlog_Builder_Item_Search_Box() );
