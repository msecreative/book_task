...
class StarterBlog_WC {
	...
	function is_active() {
		return StarterBlog()->is_woocommerce_active();
	}
	...
	function __construct() {
		if ( $this->is_active() ) {
			 ...
			 * Filter shop layout
			 *
			 * @see StarterBlog_WC::shop_layout
			 */
			add_filter( 'starterblog_get_layout', array( $this, 'shop_layout' ) );
			/**
			 * Filter special meta values for shop pages.
			 *
			 * @see StarterBlog_WC::get_post_metadata
			 */
			add_filter( 'get_post_metadata', array( $this, 'get_post_metadata' ), 999, 4 );

			add_action( 'widgets_init', array( $this, 'register_sidebars' ) );
			add_filter( 'starterblog/customizer/config', array( $this, 'customize_shop_sidebars' ) );
			add_filter( 'starterblog/sidebar-id', array( $this, 'shop_sidebar_id' ), 15, 2 );

			add_filter( 'starterblog_is_header_display', array( $this, 'show_shop_header' ), 15 );
			add_filter( 'starterblog_is_footer_display', array( $this, 'show_shop_footer' ), 15 );
			add_filter( 'starterblog_site_content_class', array( $this, 'shop_content_layout' ), 15 );
			add_filter( 'starterblog_builder_row_display_get_post_id', array( $this, 'builder_row_get_id' ), 15 );

			add_filter( 'starterblog/titlebar/args', array( $this, 'titlebar_args' ) );
			add_filter( 'starterblog/titlebar/config', array( $this, 'titlebar_config' ), 15, 2 );
			add_filter( 'starterblog/titlebar/is-showing', array( $this, 'titlebar_is_showing' ), 15 );
			add_filter( 'starterblog/page-header/get-settings', array( $this, 'get_page_header_settings' ) );

			add_filter( 'starterblog/theme/js', array( $this, 'add_js' ) );
			...
			// Custom styling.
			add_filter( 'starterblog/styling/primary-color', array( $this, 'styling_primary' ) );
			add_filter( 'starterblog/styling/secondary-color', array( $this, 'styling_secondary' ) );
			add_filter( 'starterblog/styling/link-color', array( $this, 'styling_linkcolor' ) );
			add_filter( 'starterblog/styling/color-border', array( $this, 'styling_border_color' ) );
			add_filter( 'starterblog/styling/color-meta', array( $this, 'styling_meta_color' ) );

			// Shopping Cart.
			require_once get_template_directory() . '/inc/compatibility/woocommerce/config/header/cart.php';
			add_filter( 'woocommerce_add_to_cart_fragments', array( $this, 'cart_fragments' ) );
			add_filter( 'StarterBlog_JS', array( $this, 'StarterBlog_JS' ) );
			...
			add_action( 'starterblog_wc_loop_start', array( $this, 'loop_start' ) );
			...
			add_filter( 'woocommerce_product_categories_widget_args', array( $this, 'starterblog_wc_cat_list_args' ) );
	...
	function woocommerce_get_script_data( $data, $handle ) {
		if ( 'woocommerce' == $handle ) {
			$data['qty_pm'] = apply_filters( 'starterblog_qty_add_plus_minus', 1 );
		}
	...
	function loop_start() {
		...
		wc_set_loop_prop( 'media_secondary', StarterBlog()->get_setting( 'wc_cd_media_secondary' ) );
		...
		if ( ! $name ) { // Main loop.
			...
			wc_set_loop_prop( 'mobile_columns', StarterBlog()->get_setting( 'woocommerce_catalog_mobile_columns' ) );

		} elseif ( 'related' == $name || 'up-sells' == $name || 'cross-sells' == $name ) {

			if ( 'up-sells' == $name ) {
				$columns = StarterBlog()->get_setting( 'wc_single_product_upsell_columns', 'all' );
			} else {
				$columns = StarterBlog()->get_setting( 'wc_single_product_related_columns', 'all' );
			}
	...
	function related_products_args( $args ) {
		$args['posts_per_page'] = StarterBlog()->get_setting( 'wc_single_product_related_number' );

		return $args;
	}

	function maybe_disable_related() {
		$n = StarterBlog()->get_setting( 'wc_single_product_related_number' );
		if ( 0 == $n ) {
	...
	function maybe_disable_upsell() {
		$n = StarterBlog()->get_setting( 'wc_single_product_upsell_number' );
		if ( 0 == $n ) {
	...
	function updsell_products_args( $args ) {
		$args['posts_per_page'] = StarterBlog()->get_setting( 'wc_single_product_upsell_number' );
	...
	function custom_styles( $enqueue_styles ) {
		...
		$enqueue_styles['woocommerce-general']['deps'] = 'starterblog-style';
	...
	/**
	 * Add more settings to StarterBlog JS
	 *
	 * @param array $args JS settings.
	 *
	 * @return mixed
	 */
	function StarterBlog_JS( $args ) {
	...
	function cart_fragments( $cart_fragments = array() ) {
		...
		$cart_fragments['.starterblog-wc-sub-total'] = '<span class="starterblog-wc-sub-total">' . $sub_total . '</span>';
		...
		$cart_fragments['.starterblog-wc-total-qty'] = '<span class="' . $class . '">' . $qty . '</span>';
	...
	function shop_content_layout( $classes = array() ) {
		if ( $this->is_shop_pages() ) {
			$page_layout = $this->get_shop_page_meta( '_starterblog_content_layout' );
			if ( $page_layout ) {
	...
	function show_shop_header( $show = true ) {
		if ( $this->is_shop_pages() ) {
			$disable = $this->get_shop_page_meta( '_starterblog_disable_header' );
			if ( $disable ) {
				$show = false;
			}
		}
	...
	function show_shop_footer( $show = true ) {
		if ( $this->is_shop_pages() ) {
			...
			foreach ( $rows as $row_id ) {
				if ( ! starterblog_is_builder_row_display( 'footer', $row_id, $shop_id ) ) {
					$count ++;
				}
			}
			if ( $count >= count( $rows ) ) {
	...
	function show_shop_title( $show = true ) {
		if ( $this->is_shop_pages() ) {
			$disable = $this->get_shop_page_meta( '_starterblog_disable_page_title' );
			if ( $disable ) {
				$show = false;
			}
		}
		...
		return apply_filters( 'starterblog_is_shop_title_display', $show );
	}
	...
	function get_page_header_settings( $args ) {
		if ( is_product_taxonomy() ) {
			...
			$image        = StarterBlog()->get_media( $thumbnail_id, 'full' );
			if ( $image ) {
				$args['image'] = $image;
			}
		}
	...
	function titlebar_is_showing( $show = true ) {
		...
		if ( is_shop() ) {
			...
			if ( StarterBlog_Breadcrumb::get_instance()->support_plugins_active() && ! StarterBlog()->get_setting( 'titlebar_display_product' ) ) {
				$show = false;
			} else {
				$show = true;
			}
			if ( StarterBlog()->is_using_post() ) {
				$breadcrumb_display = get_post_meta( wc_get_page_id( 'shop' ), '_starterblog_breadcrumb_display', true );
			...
			if ( StarterBlog()->get_setting( 'titlebar_display_product_tax' ) ) {
				$show = true;
			} else {
				$show = false;
			}
		} elseif ( is_product() ) {
			if ( StarterBlog()->get_setting( 'titlebar_display_product' ) ) {
				$show = true;
			} else {
				$show = false;
			}
	...
	function titlebar_config( $config, $titlebar ) {
		...
		$config[] = array(
			...
			'default'        => 1,
			'section'        => $section,
			'checkbox_label' => __( 'Display on product taxonomies (categories/tags,..)', 'starter-blog' ),
		);
		...
		$config[] = array(
			...
			'default'        => 1,
			'section'        => $section,
			'checkbox_label' => __( 'Display on single product', 'starter-blog' ),
		);
	...
	function register_sidebars() {
		register_sidebar(
			array(
				'name'          => esc_html__( 'WooCommerce Primary Sidebar', 'starter-blog' ),
				'id'            => 'shop-sidebar-1',
				'description'   => esc_html__( 'Add widgets here.', 'starter-blog' ),
		...
		register_sidebar(
			array(
				'name'          => esc_html__( 'WooCommerce Secondary Sidebar', 'starter-blog' ),
				'id'            => 'shop-sidebar-2',
				'description'   => esc_html__( 'Add widgets here.', 'starter-blog' ),
				'before_widget' => '<section id="%1$s" class="widget %2$s">',
				'after_widget'  => '</section>',
	...
	function get_post_metadata( $value, $object_id, $meta_key, $single ) {
		$meta_keys = array(
			'_starterblog_page_header_display' => '',
			'_starterblog_breadcrumb_display'  => '',
		);
		...
		if ( wc_get_page_id( 'cart' ) == $object_id || wc_get_page_id( 'checkout' ) == $object_id ) {
			...
			switch ( $meta_key ) {
				case '_starterblog_page_header_display':
					if ( empty( $value ) || 'default' == $value[0] || 'normal' == $value[0] || ! $value[0] ) {
						$value[0] = 'normal';
					}
					break;
				case '_starterblog_breadcrumb_display':
	...
	function shop_layout( $layout = false ) {
		if ( $this->is_shop_pages() ) {
			...
			$page_custom = get_post_meta( $page_id, '_starterblog_sidebar', true );
			if ( $page_custom ) {
				$layout = $page_custom;
			} elseif ( $page ) {
				$layout = $page;
		...
		if ( is_product() ) {
			$product_sidebar = get_post_meta( get_the_ID(), '_starterblog_sidebar', true );
			if ( ! $product_sidebar ) {
				$product_custom = StarterBlog()->get_setting( 'product_sidebar_layout' );
...
function StarterBlog_WC() {
	return StarterBlog_WC::get_instance();
}
 ...
 * @return string
 */
function starterblog_get_default_catalog_view_mod() {
	$name    = wc_get_loop_prop( 'name' );
	$default = StarterBlog()->get_setting( 'wc_cd_default_view' );
	if ( $name ) {
		return apply_filters( 'starterblog_get_default_catalog_view_mod', 'grid' );
	}
	...
	if ( ! StarterBlog()->get_setting( 'wc_cd_show_view_mod' ) ) {
		$use_cookies = false;
	}

	if ( $use_cookies ) { // Do not use cookie in customize.
		$cookie_mod = ( isset( $_COOKIE['starterblog_wc_pl_view_mod'] ) && $_COOKIE['starterblog_wc_pl_view_mod'] ) ? sanitize_text_field( $_COOKIE['starterblog_wc_pl_view_mod'] ) : false; // WPCS: sanitization ok.
		if ( $cookie_mod ) {
	...
	return apply_filters( 'starterblog_get_default_catalog_view_mod', $default );
}
